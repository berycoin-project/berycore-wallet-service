...
helpers._generateCopayersTestData = function() {
  ...
  _.each(xPrivKeys, function(xPrivKeyStr, c) {
    ...
    var id45 = Model.Copayer._xPubToCopayerId('bery', xpub_45H.toString());
    ...
    var id44bery = Model.Copayer._xPubToCopayerId('bery', xpub_44H_0H_0H.toString());
...
helpers.toSatoshi = function(bery) {
  if (_.isArray(bery)) {
    return _.map(bery, helpers.toSatoshi);
  } else {
    return Utils.strip(bery * 1e8);
  }
};
...
helpers._parseAmount = function(str) {
  ...
  var re = /^((?:\d+c)|u)?\s*([\d\.]+)\s*(bery|bit|sat)?$/;
